<template>
  <div class="app-container" :style="'height:' + (clientHeight - 50) + 'px;'">
    <el-row :gutter="0" class="row">
      <!-- 左边栏 -->
      <el-col :xs="24" :span="6" class="column">
        <el-table
...
export default {
  ...
  methods: {
    ...
    scrollToBottom() {
      const _this = this;
      this.$nextTick(() => {
        const scrollContainer = _this.$el.querySelector(
    ...
    getConversationList() {
      conversationApi
        .getConversationList(this.$store.getters.id)
        .then(response => {
    ...
    listenMessageScroll() {
      ...
      setTimeout(function() {
        ...
        scrollContainer.onscroll = function() {
          console.log(
            `滚动中 scrollTop=${scrollContainer.scrollTop}, scrollHeight=${scrollContainer.scrollHeight}, clientHeight=${scrollContainer.clientHeight}`
          );
