  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="性别" min-width="100px" align="center">
        <template slot-scope="{row}">
          <el-tag :type="row.gender | genderTagFilter">{{ row.gender | genderNameFilter }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="状态" min-width="100px" align="center">
        <template slot-scope="{row}">
          <el-tag :type="row.status | statusTagFilter">{{ row.status | statusNameFilter }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="微信登录" min-width="100px" align="center">
        <template slot-scope="{row}">
          <el-tag
            :type="row.openId != null && row.openId.length === 0 ? 'info' : 'success'"
          >{{ row.openId != null && row.openId.length === 0 ? '未开通' : '已开通' }}</el-tag>
        </template>
      </el-table-column>
